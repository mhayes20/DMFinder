...
sub checkLinks
{
	...
	for(my $i = 0; $i < scalar(@$SVlinks); $i++)
	{
		...
		if(($chr eq $rec[0] && abs($amp1 - $rec[1]) <= $window) || ($chr eq $rec[3] && abs($amp1 - $rec[4]) <= $window) || ($chr eq $rec[0] && abs($amp2 - $rec[1]) <= $window) || ($chr eq $rec[3] && abs($amp2 - $rec[4]) <= $window)) 
		{
			#print "IN IF\n";
			# print "CHR is $chr and rec[0] is ".$rec[0]."\n"; 
			return 1;	
...
for(my $i = 0; $i < scalar(@filerec)-1; $i++)
{
	...
	if((abs($record[9] - $record2[8]) < 10000 && $chr eq $record2[7] ) )
	{
		...
		$new_start = $record[8]; #Start of merged amplicon will be the start of the first original amplicon
